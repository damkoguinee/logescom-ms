  ...
  if ($products['level']>=3) {
    ...
    if (!isset($_GET['ajout']) ) {?>  
      ...
      <table class="payement">
        ...
        <thead>
          <tr><th class="legende" colspan="12" height="30"><?="Liste des dépôts " .$datenormale ?> <a style="color:orange; font-size:30px;" href="versement.php?ajout">Effectuer un dépôt</a></th></tr>
        ...
        <tbody><?php
          ...
          if (isset($_POST['j1'])) {
            ...
            if ($_SESSION['level']>6) {
              $products= $DB->query('SELECT versement.id as id, client.id as idc, numcmd, client.nom_client as nom_client, montant, motif, type_versement, devisevers, date_versement FROM versement inner join client on client.id=versement.nom_client  order by(versement.id) desc ');
            }else{

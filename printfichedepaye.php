...
if (isset($_GET['idc'])) {
	...
	$prodpaye=$DB->querys("SELECT sum(montant) as montant, sum(montantav) as montantav, sum(montantpr) as montantpr, sum(montantcot) as montantcot, date_payement, client, numdec, payement, devisedep FROM decdepense where client='{$_GET['idc']}' and DATE_FORMAT(periodep, \"%m\")='{$mois}' ");    

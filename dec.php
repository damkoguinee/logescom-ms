      ...
      if (isset($_POST['clientliv'])) {
        ...
        if (!isset($_GET['ajout'])) {?>
            ...
            <table class="payement">
              ...
              <thead>
                <tr><th class="legende" colspan="12" height="30"><?="Liste des Décaissements " .$datenormale ?> <a href="dec.php?ajout" style="color:orange; font-size:30px;">Effectuer un décaissement</a></th></tr>
              ...
              <tbody><?php 
                ...
                if (isset($_POST['j1'])) {
                  ...
                  if (isset($_POST['magasin']) and $_POST['magasin']=='general') {
                    ...
                    $products= $DB->query('SELECT decaissement.id as id, client.id as idc, numdec, client.nom_client as client, payement as type, montant, coment, payement, devisedec, DATE_FORMAT(date_payement, \'%d/%m/%Y\')AS DateTemps FROM decaissement left join client on client.id=decaissement.client  WHERE lieuvente=:lieu and DATE_FORMAT(date_payement, \'%Y%m%d\')>= :date1 and DATE_FORMAT(date_payement, \'%Y%m%d\')<= :date2 order by(decaissement.id) ', array('lieu'=>$_POST['magasin'], 'date1' => $_SESSION['date1'], 'date2' => $_SESSION['date2']));
                  }                 

                }elseif (isset($_GET['searchversclient'])) {

                  if ($_SESSION['level']>6) {
                    $products= $DB->query('SELECT decaissement.id as id, client.id as idc, numdec, client.nom_client as client, payement as type, montant, coment, payement, devisedec, DATE_FORMAT(date_payement, \'%d/%m/%Y\')AS DateTemps FROM decaissement inner join client on client.id=decaissement.client  WHERE decaissement.client = :client order by(decaissement.id)', array('client' => $_GET['searchversclient']));
                  }else{
